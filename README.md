...
3. ツールバーの「文字」「長方形」「楕円」「線分」を選び、加工エリアをクリックして追加します。「描く」はフリーハンドで曲線を描きます（下記）。オブジェクトはドラッグで自由配置できます。
...
- ピンチまたはCtrl/⌘＋ホイールで25〜2000%にズームします。タッチ画面は2本指で移動・ピンチできます。スマートフォンでは下部バーの「全体」で 100%（加工エリア全体）に戻ります。
...
- スマートフォン・タブレットは専用のレイアウトになります（下記「スマートフォン・タブレットでの操作」）。

### フリーハンドで描く（描く）

ツールバーの「描く」（スマートフォンではキャンバス右下の「✎ 描く」）を選び、ペン・指・マウスでキャンバスをドラッグすると、その線が**なめらかなベジェ曲線の固定パス**になります。

- 手ぶれは自動で取り除きます。はっきり折れた角（60°以上）は角のまま残り、それより緩い曲がりは曲線になります。
- 始点の近くで描き終える（または始点を通り過ぎる）と閉じた図形になり、切り抜く形として扱います。そうでなければ開いた線（カット線）です。右側の「始点の近くで終えたら閉じる」で無効にできます。
- 「なめらかさ」（0〜10、既定 3）は**画面上のピクセル**単位です。拡大して描くほど細かい形が残り、縮小して描くほど滑らかになります。値はこのブラウザに記憶します。
- 描いた線は固定パスなので、ダブルクリック（ダブルタップ）でノードを編集でき、ブリッジ・図形演算・ワープ・SVG書き出しにそのまま使えます。
- 「描く」中に動かさずにタップ（クリック）すると、その場所のオブジェクトを選択します。ペンで描いている間は、画面に触れた手のひらでズームが始まらないようにしています。

### スマートフォン・タブレットでの操作

幅 860 px 以下の画面では、MandalaFab と同じ構成のレイアウトになります。

- **上部**: 元に戻す・やり直す、「⋯」メニュー（新規・開く・保存・SVG書き出し・加工注文・ブリッジ・スマート接続・アウトライン化・グループ化・ワープ・パス編集・加工プレビュー・使い方・フォントライセンス）、「?」（使い方）、「SVG」（書き出し）。
- **キャンバス**: 画面いっぱいに表示します。右下の「✎ 描く」「↖ 選択」と、ズームの − / % / + ボタン。1本指のドラッグは移動・範囲選択、2本指でスクロールとピンチズームです。オブジェクトを**長押し**すると編集メニュー（右クリックメニューと同じ）が開きます。
- **下部のバー**: 「オブジェクト」（ブラウザ）、「ツール」（スケッチツールと 2D CAD ツール）、「編集」（プロパティ。選択数をバッジで表示）、「プレビュー」、「全体」（100% に戻す）。オブジェクト・ツール・編集は画面下から出るシートで、× か同じボタンで閉じます。ツールを選ぶとシートは自動で閉じます。
- メッセージは画面上部に数秒表示します。
...
自動テストではブリッジの線分除去、回転、複数隙間、自動配置、両書体の日本語輪郭、SVG形式、JSON入力検証、フリーハンドの平滑化・角の検出・閉合・ベジェ近似を確認します。ブラウザの操作確認とGitHub Pagesへの公開確認は `PLANS.md` に記録します。実機レーザー加工・材料強度・加工ソフト全種類での互換性は未検証です。
