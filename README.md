...
6. 「加工プレビュー」で赤いカット線と線の途切れを確認し、「SVGを書き出す」で保存します。Chrome・Edgeでは保存ダイアログが開き、ファイルの種類は「.svg」に固定されます。そのほかのブラウザでは「typefab.svg」としてダウンロードします（プロジェクトの「保存」は「typefab-project.json」）。
